...
public class PerlinNoise
{
	...
	{
		float noise = perlinNoise(x, z, octaves, persistence) + .6f;
		...
		if(distFromCenterNormalized < .05 && noise < 0)
			noise = .05f;
		distFromCenterNormalized = (float) Math.pow(distFromCenterNormalized, .6);	//Makes the effect increase much more as you get further away from center
		noise -= distFromCenterNormalized * 1.4;
		return noise;
	}
}
