				...
				if (world.getBiome(worldChunkX + x, worldChunkZ + z) == Biome.SWAMPLAND)
				{
					for (y = 128; y > 0; --y)
					{
						...
						if (block.getType() == Material.AIR
								&& ground.getType() == Material.GRASS
								&& block.getLightLevel() <= 8
								&& this.random.nextInt(100) < 5)
						{
							if (world.getHighestBlockYAt(x + worldChunkX, z
									+ worldChunkZ) > y)
							{
								block.setType((this.random.nextInt(100) < 20) ? Material.RED_MUSHROOM
