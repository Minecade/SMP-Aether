...
public class LakePopulator extends BlockPopulator
{
	...
	public void populate(World world, Random random, Chunk chunk)
	{
		...
		if (this.random.nextInt(4) == 0)
		{
			...
			if (Arrays.asList(Biome.JUNGLE, Biome.JUNGLE_HILLS).contains(
					world.getBiome(x, z)) == false)
			{
				y = world.getHighestBlockYAt(x, z) + 2;

				if (this.random.nextInt(100) < 85)
				{
					(new WorldGenLakes(Material.STATIONARY_WATER.getId())).a(
							mcWorld, this.random, x, y, z);
					(new WorldGenReed()).a(mcWorld, this.random, x, y, z);
				} else
				{
					(new WorldGenLakes(Material.STATIONARY_LAVA.getId())).a(
							mcWorld, this.random, x, y, z);
				}
			}
		}
